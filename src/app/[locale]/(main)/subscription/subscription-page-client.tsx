...
export function SubscriptionPageClient({ plans, packs }: SubscriptionPageClientProps) {
  ...
  useEffect(() => {
    ...
    if (searchParams.get("success") === "true") {
      toast.success(t("subscription:messages.subscriptionSuccess"));
      toastShown.current = true;
    } else if (searchParams.get("canceled") === "true") {
      toast.info(t("subscription:messages.paymentCanceled"));
      toastShown.current = true;
    }
  }, [searchParams, t]);
  ...
  async function handleSubscribe(planId: string) {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(json.error ?? t("subscription:messages.unexpectedError"));
        return;
      }
      ...
      toast.error(t("subscription:messages.connectionError"));
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="mb-10 text-center">
        <h1 className="text-3xl font-bold md:text-4xl">{t("subscription:pageTitle")}</h1>
            ...
            <CardContent className="flex flex-col items-center gap-4 p-6 sm:flex-row sm:justify-between">
              <div className="text-center sm:text-start">
                ...
                <div className="text-muted-foreground mt-1 flex items-center justify-center gap-1.5 text-sm sm:justify-start">
                  ...
                  <span>
                    {t("subscription:labels.remainingBalance")}{" "}
                    <span className="text-foreground font-semibold">
                      {credits?.total ?? 0}
                    </span>{" "}
                    {t("subscription:labels.credits")}
                  </span>
                </div>
                {subscriptionData.currentPeriodEnd && (
                  <p className="text-muted-foreground mt-1 text-sm">
                    {t("subscription:labels.renewal")}{" "}
                    {new Date(subscriptionData.currentPeriodEnd).toLocaleDateString(i18n.language === "ar" ? "ar-SA" : "en-US")}
                  </p>
                )}
              </div>
              <Button
              ...
              >
                {isManaging ? (
                  ...
                  <>
                    ...
                    {t("subscription:buttons.manageSubscription")}
                  </>
                )}
              </Button>
            </CardContent>
      ...
      {packs.length > 0 && (
        <div id="topup" className="mt-16">
          <div className="mb-8 text-center">
            <h2 className="text-2xl font-bold">{t("subscription:topup.title")}</h2>
